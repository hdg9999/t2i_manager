...
def tag_manager():
    st.subheader('태그 관리', divider=True)  
...
def search_options():
    st.divider()
    with st.expander(label='검색 옵션'):    
        st.session_state.top_K = st.slider(label='검색 갯수', min_value=1, max_value=1000, value=10)